...
class ProgramNode(object):
    ...
    def __str__(self):
        if len(self.children) == 1:
            ...
            return "{}{{{}; {}; {}}}".format(
                self.name, self.children[0], self.children[1], self.children[2]
            )
        else:
        ...
        if tree1.name == tree2.name:
            if len(tree1.children) == len(tree2.children):
                if tree1.name in ["eq", "and"]:
                    t1 = tree_eq(tree1.children[0], tree2.children[0]) and tree_eq(
                        tree1.children[1], tree2.children[1]
                    )
                    t2 = tree_eq(tree1.children[0], tree2.children[1]) and tree_eq(
                        tree1.children[1], tree2.children[0]
                    )
                    return t1 or t2
                else:
                    return all(
        ...
        for root in hist[step]:
            ...
            for k, v in APIs.items():
                ...
                # Incrementing/Decrementing/Whether is none
                    ...
                    if step == 0:
                        if len(root.memory_str) > 1 and all(
                            ["tmp_" not in h for h, _ in root.memory_str]
                        ):
                            if len(set([h for h, _ in root.memory_str])) == 1:
                                command = v["tostr"](
                                    [
                                        root.trace_str[i]
                                        for i in range(len(root.memory_str))
                                    ]
                                )
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
                                    ...
                                    returned = call(
                                        command,
                                        v["function"],
                                        [va for h, va in root.memory_str],
                                    )
                                    if tmp.done():
                                        tmp.append_result(command, returned)
                                        finished.append((tmp, returned))
                        ...
                        for i, (h, va) in enumerate(root.memory_str):
                            ...
                            for head in root.header_str:
                                ...
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
                                    returned = call(
                                        command, v["function"], row, head, va
                                    )
                                    if v["output"] == "bool":
                    ...
                    if root.memory_bool_len < 2:
                        ...
                        for l in range(0, root.memory_bool_len - 1):
                            for m in range(l + 1, root.memory_bool_len):
                                command = v["tostr"](
                                    root.memory_bool[l][0], root.memory_bool[m][0]
                                )
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
                                    returned = call(
                                        command,
                                        v["function"],
                                        root.memory_bool[l][1],
                                        root.memory_bool[m][1],
                                    )
                                    if v["output"] == "bool":
                        ...
                        for j, (row_h, row) in enumerate(root.rows):
                            ...
                            for l in range(len(root.header_num)):
                                ...
                                if not root.exist(command):
                                    ...
                                    tmp.inc_row_counter(j)
                                    returned = call(
                                        command, v["function"], row, root.header_num[l]
                                    )
                                    if v["output"] == "num":
                                        tmp.add_memory_num(
                                            "tmp_" + root.header_num[l],
                                            returned,
                                            command,
                                        )
                                        conditional_add(tmp, hist[step + 1])
                    ...
                    if "most_freq" in k:
                        ...
                        for l in range(len(root.header_str)):
                            ...
                            if not root.exist(command):
                                tmp = root.clone(command, k)
                                returned = call(
                                    command, v["function"], row, root.header_str[l]
                                )
                                if v["output"] == "str":
                                    if returned is not None:
                                        tmp.add_memory_str(
                                            "tmp_" + root.header_str[l],
                                            returned,
                                            command,
                                        )
                                        conditional_add(tmp, hist[step + 1])
                    ...
                    for l in range(0, root.memory_num_len - 1):
                        for m in range(l + 1, root.memory_num_len):
                            if (
                                "tmp_" in root.memory_num[l][0]
                                or "tmp_" in root.memory_num[m][0]
                            ):
                                if (
                                    "tmp_input" == root.memory_num[l][0]
                                    and "tmp_" not in root.memory_num[m][0]
                                ) or (
                                    "tmp_input" == root.memory_num[m][0]
                                    and "tmp_" not in root.memory_num[l][0]
                                ):
                                    continue
                                elif (
                                    root.memory_num[l][0]
                            ...
                            if v["output"] == "num":
                                if type_l == type_m:
                                    ...
                                    for dir1, dir2 in zip([l, m], [m, l]):
                                        command = v["tostr"](
                                            root.trace_num[dir1], root.trace_num[dir2]
                                        )
                                        tmp = root.clone(command, k)
                                        ...
                                        returned = call(
                                            command,
                                            v["function"],
                                            root.get_memory_num(dir1),
                                            root.get_memory_num(dir2),
                                        )
                                        tmp.add_memory_num(
                                            "tmp_" + root.memory_num[dir1][0],
                                            returned,
                                            command,
                                        )
                    ...
                    for l in range(0, root.memory_str_len - 1):
                        for m in range(l + 1, root.memory_str_len):
                            if (
                                "tmp_" not in root.memory_str[l][0]
                                and "tmp_" not in root.memory_str[m][0]
                            ):
                                continue
                            ...
                            if type_l == type_m:
                                command = v["tostr"](
                                    root.trace_str[l], root.trace_str[m]
                                )
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
                                    ...
                                    if v["output"] == "bool":
                                        returned = call(
                                            command,
                                            v["function"],
                                            root.get_memory_str(m),
                                            root.get_memory_str(l),
                                        )
                                        if tmp.done():
                                            tmp.append_result(command, returned)
                                            finished.append((tmp, returned))
                    ...
                    for i, (h1, va1) in enumerate(root.memory_str):
                        for j, (h2, va2) in enumerate(root.memory_num):
                            if "tmp_" not in h1 and "tmp_" not in h2:
                                command = v["tostr"](
                                    h1, root.trace_str[i], h2, root.trace_num[j]
                                )
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
                                    tmp.delete_memory_str(i)
                                    ...
                                    returned = call(
                                        command, v["function"], row, h1, va1, h2, va2
                                    )
                                    if v["output"] == "bool":
                                        if tmp.done():
                                            tmp.append_result(command, returned)
                    ...
                    for l in range(len(root.memory_str) - 1):
                        for m in range(l + 1, len(root.memory_str)):
                            ...
                            if "tmp_" not in h1 and "tmp_" not in h2 and (h1 != h2):
                                command = v["tostr"](
                                    h1, root.trace_str[l], h2, root.trace_str[m]
                                )
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
                                    tmp.delete_memory_str(l, m)
                    ...
                    for l in range(len(root.memory_num) - 1):
                        for m in range(l + 1, len(root.memory_num)):
                            ...
                            if ("tmp_" not in h1 and "tmp_" not in h2) and (h1 != h2):
                                command = v["tostr"](
                                    h1, root.trace_num[l], h2, root.trace_num[m]
                                )
                                if not root.exist(command):
                                    tmp = root.clone(command, k)
...
class Parser(object):
    ...
    def initialize_buffer(self, table_name, sent, pos_tag, raw_sent):
        ...
        for i in range(len(tokens)):
            ...
            if any(
                [
                    _ in tokens
                    for _ in ["than", "over", "more", "less", "below", "above", "least"]
                ]
            ):
            ...
            if (
                i > 1
                and tokens[i - 1] in ["be", "are", "is"]
                and tokens[i - 2] == "there"
            ):
                count_features.append(4)
                compare_features.append(-4)
        ...
        for index, (k, v) in enumerate(mem_num):
            if k != "tmp_input" and isinstance(v, int) and v < 10:
                if (
                    "there be {} ".format(v) in raw_sent
                    or "there are {} ".format(v) in raw_sent
                    or "there is {} ".format(v) in raw_sent
                    or " {} of ".format(v) in raw_sent
                    or raw_sent.startswith(str(v) + " ")
                ):
        ...
        if (
            any([_ in pos_tag for _ in ["RBR", "RBS", "JJR", "JJS"]])
            and len(head_num) == 0
        ):
            for col_idx, k in zip(range(len(cols) - 1, -1, -1), cols[::-1]):
                if mapping[col_idx] == "num":
    ...
    def run(
        ...
        t = pandas.read_csv(
            os.path.join(self.folder, table_name), delimiter="#", encoding="utf-8"
        )
        t.fillna("")
    ...
    def parse(self, table_name, sent, debug=False):
        ...
        (
            ...
            head_num,
            mapping,
        ) = self.initialize_buffer(table_name, linked_sent, pos, raw_sent)
        ...
        result = self.run(
            table_name, raw_sent, masked_sent, pos, mem_str, mem_num, head_str, head_num
        )
    ...
    def distribute_parse(self, inputs):
        ...
        if not os.path.exists("tmp/results/{}.json".format(hash_id)):
            ...
            with open("tmp/results/{}.json".format(hash_id), "w") as f:
                json.dump(
                    (inputs[1], result, self.title_mapping[table_name]), f, indent=2
                )
