...
bleu_script = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "files", "mteval-v13m.pl")
)
meteor_jar = os.path.abspath(
    os.path.join(os.path.dirname(os.path.dirname(__file__)), "meteor-1.4.jar")
)
...
def main(argv):
    ...
    if not os.path.exists(meteor_jar):
        print "Please edit the meteor_jar line of {0} to reflect the location of meteor-*.jar".format(
            __file__
        )
        sys.exit(1)
    ...
    # Usage
    if len(argv[1:]) < 4:
        print "usage: {0} <lang> <sys1.hyp> <sys2.hyp> <ref1> [ref2 ...]".format(
            argv[0]
        )
    ...
    ref_len = write_sgm(
        ref_files[0],
        src_sgm,
        '<srcset trglang="any" setid="any" srclang="any">',
        "</srcset>",
    )
    ...
    write_ref_sgm(
        ref_files,
        ref_sgm,
        '<refset trglang="any" setid="any" srclang="any">',
        "</refset>",
    )
    ...
    # Footer
    print "+-------------+--------+----------+"
    print "|  System2 +  | {0:6} |   {1:6} |".format(
        sum(bleu_dd[11:]), sum(meteor_dd[11:])
    )
    print "|  System2 -  | {0:6} |   {1:6} |".format(
        sum(bleu_dd[0:10]), sum(meteor_dd[0:10])
    )
    print "+-------------+--------+----------+"
...
def bleu(hyp, ref, src, work_dir=os.curdir):
    ...
    subprocess.Popen(
        bleu_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=work_dir
    ).wait()
...
def meteor(hyp, ref, lang="en", work_dir=os.curdir):
    # Run Meteor
    meteor_cmd = ["java", "-Xmx2G", "-jar", meteor_jar, hyp, ref, "-sgml", "-l", lang]
    subprocess.Popen(
        meteor_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=work_dir
    ).wait()
    # Get scores from file
    seg = {}
