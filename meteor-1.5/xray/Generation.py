...
def check_printable(a1, a2=None):
    ...
    if len(a1.sen2) > MAX_LEN:
        print >> sys.stderr, "Skipping", a1.name, "- too large:", len(
            a1.sen2
        ), "reference words"
        return False
    # Different references?
    if a2 and a1.sen2 != a2.sen2:
        print >> sys.stderr, "Skipping", a1.name, "- different references used"
        return False
    return True
...
def print_align_table(tex_out, a1, a2=None, a_type=ALIGN_METEOR):
    ...
    for i in range(len(a1.sen2)):
        if i >= half:
            ...
            g += step * 1.0
            b -= step * 0.5
        print >> tex_out, r"\definecolor{{ref{0}}}{{rgb}}{{{1},{2},{3}}}".format(
            i, min(1.0, r), min(1.0, g), min(1.0, b)
        )
    # Print table start
    ...
    for i in range(len(a1.sen2)):
        ...
        line += (
            r"&\begin{sideways}"
            + r"\cellcolor{{ref{0}}}".format(i)
            + w2
            + "\hspace{12pt}\end{sideways}"
        )
    if a2:
        line += r"&\rex \rap"
    ...
    # Compare stats
        ...
        if a2:
            ...
            print >> tex_out, r"\hline"
            print >> tex_out, r"P:&&{0:.3f}&\centering vs&{1:.3f}&\centering :&{{\bf\color{{{2}}}{{{3:.3f}}}}}\\".format(
                a1.p, a2.p, "gb" if p_diff >= 0 else "rb", p_diff
            )
            print >> tex_out, r"R:&&{0:.3f}&\centering vs&{1:.3f}&\centering :&{{\bf\color{{{2}}}{{{3:.3f}}}}}\\".format(
                a1.r, a2.r, "gb" if r_diff >= 0 else "rb", r_diff
            )
            print >> tex_out, r"Frag:&&{0:.3f}&\centering vs&{1:.3f}&\centering :&{{\bf\color{{{2}}}{{{3:.3f}}}}}\\".format(
                a1.frag, a2.frag, "rb" if fr_diff > 0 else "gb", fr_diff
            )
            print >> tex_out, r"Score:&&{0:.3f}&\centering vs&{1:.3f}&\centering :&{{\bf\color{{{2}}}{{{3:.3f}}}}}\\".format(
                a1.score, a2.score, "gb" if sc_diff >= 0 else "rb", sc_diff
            )
        else:
...
def write_plot_hist(
    work_dir, dat_file, plot_file, eps_file, xlabel="Score", num_data_cols=1
):
    ...
    print >> plot_out, GNUPLOT_HISTOGRAM.format(
        data=dat_file, eps=eps_file, label=uc_label, columns=col_line
    )
    plot_out.close()
