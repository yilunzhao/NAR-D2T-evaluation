...
def main(argv):
    ...
    try:
        ...
        print >> sys.stderr, "Dir {0} exists, will overwrite contents".format(pre_dir)
    ...
    if compare:
        ...
        alignments.sort(
            cmp=cmp_score_best if best_first else cmp_score_diff, reverse=True
        )
        if not no_align:
            # Write tex file
            ...
            print >> tex_out, DOC_HEADER_COMPARE.format(
                sys1=label_list[0], sys2=label_list[1]
            )
            ...
            tex_out.close()
            # Compile pdf file
            print >> sys.stderr, "Compiling {0} - this may take a few minutes...".format(
                pdf_file
            )
    ...
    for stat in ("score", "frag", "p", "r"):
        ...
        write_dat_file(shutil.os.path.join(pre_dir, dat_file), dists, stat, label_list)
            ...
            for scores in seg_scores:
                if len(r) == 2:
                    values = [
                        eval("x." + stat)
                        for x in scores
                        if x.sen_len >= r[0] and x.sen_len <= r[1]
                    ]
                else:
                    values = [eval("x." + stat) for x in scores if x.sen_len >= r[0]]
                dists.append(get_score_dist(values))
            write_dat_file(
                shutil.os.path.join(pre_dir, dat_file), dists, stat, label_list
            )
            write_plot_hist(pre_dir, dat_file, plot_file, eps_file, stat, len(dists))
            gnuplot(pre_dir, plot_file)
    ...
    score_tex = "score.tex"
    shutil.copyfile(
        shutil.os.path.join(shutil.os.path.dirname(__file__), "template", "score.tex"),
        shutil.os.path.join(pre_dir, score_tex),
    )
    print >> sys.stderr, "Compiling {0}...".format(score_pdf)
    xelatex(score_tex, score_pdf, work_dir=pre_dir)
    print >> sys.stderr, "Supporting files written to {0}.".format(pre_dir)
