...
def main(argv):
    ...
    if len(argv[1:]) > 0:
        if argv[1] in "--help":
            ...
            print "usage:", argv[
                0
            ], "min_rel_dist", "min_seg_len", "word_list", "<", "matcher.out", ">", "matcher.out.mon"
            print ""
            print "min_rel_dist - minimum relative distance for deletion" + "(default = X)"
            print "min_seg_len  - minimum segment length (reference) to" + "consider (default = X)"
            print "word_list    - file of words, one per line, to consider" + "for deletion (default = all words)"
            sys.exit()
        else:
    ...
    while True:
        ...
        if len(words2) >= min_len and len(mods) > 1:
            ...
            for i in range(len(mods)):
                ...
                if words:
                    if (
                        words2[match_start2[i]] not in words
                ...
                if i == 0:
                    ...
                    dist_prev = abs(
                        (match_start1[i] - match_start1[i - 1])
                ...
                if i == len(mods) - 1:
                    ...
                    dist_next = abs(
                        (match_start1[i + 1] - match_start1[i])
