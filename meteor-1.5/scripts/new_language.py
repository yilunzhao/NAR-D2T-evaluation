...
METEOR_JAR = glob.glob(
    os.path.join(os.path.dirname(os.path.dirname(__file__)), "meteor-*.jar")
)[0]
JVM_SIZE = "12G"
...
def main(argv):
    ...
    if len(argv[1:]) < 4:
        sys.stderr.write(
            "usage: out-dir corpus.f corpus.e phrase-table.gz [target-corpus.e]\n".format(
                argv[0]
            )
        )
        sys.exit(2)
    ...
    shutil.copy(
        os.path.join(par_dir, "parex.e"), os.path.join(dir_files, "function.words")
    )
    sys.stderr.write("+ paraphrase.gz\n")
    par_fmt(
        os.path.join(par_dir, "paraphrase.gz"), os.path.join(dir_files, "paraphrase.gz")
    )
    sys.stderr.write(
        "{} is now ready to passed to Meteor with -new flag\n".format(dir_files)
    )
