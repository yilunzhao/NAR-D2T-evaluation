...
def main(argv):
    ...
    if len(argv[1:]) < 7:
        ...
        print >> sys.stderr, "Usage: {0} <meteor.jar> <lang> <n-mods> <task> <data_dir> <work_dir> <n-jobs> [other args like -a par.gz, -ch, ...]".format(
            argv[0]
        )
        sys.exit(1)
    ...
    for i in range(42):
        ...
        trainer_cmd = "cd {sd} && while true ; do sleep 1 ; java -Xmx1G -cp {0} Trainer {1} {2} -l {3} -i '{4}' -f '{5}' -s '{6}' {args} > {7} ; if [ \"$?\" = \"0\" ] ; then break ; fi ; done".format(
            meteor_jar,
            task,
            data_dir,
            lang,
    ...
    # Sort output
    sort_cmd = "cat {0}/output.* |sort -g -S4G --parallel={1} >{0}/output.sort".format(
        work_dir, n_jobs
    )
