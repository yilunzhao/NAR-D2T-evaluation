...
def main():
    ...
    if len(sys.argv[1:]) < 3:
        sys.stderr.write(
            "Create Meteor training sets from TransCenter reports and pre-generated (standard) references\n"
        )
        sys.stderr.write(
            "usage: {} [out-dir] report-dir1 ref1 [report-dir2 ref2 ...]\n".format(
                sys.argv[0]
            )
        )
        sys.exit(2)
    ...
    for (dir, rf) in zip(
        (os.path.abspath(dir) for dir in dirs),
        (os.path.abspath(rf) for rf in ref_files),
    ):
        sys.stderr.write("{} ({})\n".format(dir, rf))
        ...
        for f in os.listdir(dir):
            ...
            if f.startswith("summary.") and f.endswith(".csv"):
                ...
                sys.stderr.write("+ {}\n".format(user))
                headers = dict(
                    (y, x) for (x, y) in enumerate(csv.readline().strip().split("\t"))
                )
                for line in csv:
                    ...
                    for label in NORM_SCORES:
                        data[label].add(
                            mt,
                            ref_lines[id - 1],
                            float(fields[headers[label]]) / ref_lens[id - 1],
                        )
                ...
                for line in csv:
                    ...
                    for label in PAUSE_SCORES:
                        data[label].add(
                            mt_lines[id - 1], ref_lines[id - 1], fields[headers[label]]
                        )
    # Close streams
    hyps.close()
    refs.close()
