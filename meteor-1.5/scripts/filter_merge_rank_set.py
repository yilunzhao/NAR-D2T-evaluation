...
def main(argv):
    ...
    # Single rank file
    r_out = open(os.path.join(clean_dir, os.path.basename(r_files[0])), "w")
    ...
    for r_file in r_files:
        ...
        for sgm_file in os.listdir(f_dir):
            ...
            # Append rank set numbers to system names
            sgm_out = open(os.path.join(clean_dir, append_n(sgm_file, r_n)), "w")
            for line in sgm_in:
                ...
                if not r:
                    ...
                    continue
                if r.group(1) in seg:
                    print >> sgm_out, re.sub(
                        u'^<seg id="[0-9]+">',
                        '<seg id="{0}">'.format(seg[r.group(1)]),
                        line,
                    ).strip()
            sgm_in.close()
            sgm_out.close()
