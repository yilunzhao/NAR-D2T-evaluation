...
def _table_reader_v2(single_table_str):
  """
  Input: String, representing one table 
  Output: Yields a table parsed into a list of tuples with tokenized entries.
  """
  entries = single_table_str.lower().split("<tab>")
  table = [
      [member.split() for member in entry.split("|||")] for entry in entries
  ]
  yield table
