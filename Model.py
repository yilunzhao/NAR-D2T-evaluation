...
class TableDecoder(nn.Module):
    ...
    def forward(self, tgt_seq, src_feat, tgt_feat):
        ...
        slf_attn_mask = (
            torch.zeros_like(tgt_seq)
            .unsqueeze(2)
            .repeat(1, 1, tgt_length)
            .type(torch.bool)
            .to(tgt_seq.device)
        )
...
class TableInfusing(nn.Module):
    def __init__(
        self, vocab_size, full_vocab_size, d_word_vec, n_layers, n_head, dropout=0.1
    ):
    ...
    def forward(
        self, seqs_in, table_in, table_scatters, lookups, line_nos, fields, indexes
    ):
    ...
    def encode(self, table_in, lookups, line_nos, fields, indexes):
        ...
        extracted = torch.gather(
            out, 1, indexes[:, :, None].repeat(1, 1, out.shape[-1])
        )

        field_emb = torch.gather(
            extracted, 1, lookups[:, :, None].repeat(1, 1, extracted.shape[-1])
        )
    ...
    def decode(self, seqs_in, enc_inp, table_scatters):
        ...
        dec_enc_attn_mask = (
            src_slf_mask.unsqueeze(1).expand(batch_size, length, -1).type(torch.bool)
        )
        ...
        add_on = (
            torch.FloatTensor(batch_size, length, size)
            .fill_(add_on_prob)
            .to(in_vocab_prob.device)
        )

        full_prob = torch.cat([in_vocab_prob * (1 - gate) * self.discount, add_on], -1)

        full_prob = full_prob.scatter_add(
            2, table_scatters.unsqueeze(1).repeat(1, length, 1), oov_vocab_prob * gate
        )
