...
APIs["filter_greater_eq"] = {
    "argument": ["row", ["header_num", "num"]],
    "output": "row",
    "function": lambda t, col, value: t[t[col] >= value],
    "tostr": lambda t, col, value: "filter_greater_eq{{{}; {}; {}}}".format(
        t, col, value
    ),
    "append": False,
}
...
def inner(t, t1):
    ...
    idx = t.loc[
        (t[col1] == val1) & (t[col2] == val2) & (t[col3] == val3) & (t[col4] == val4)
    ].index
    if len(idx) > 0:
        return idx[0].item()
    else:
...
non_triggers["only"] = ["REG:only [^2-9]", "unique", "except"]
...
non_triggers["all_eq"] = ["all", "every", "each", "only", "always"]
non_triggers["all_less"] = [
    ["all", "every", "each"],
    ["RBR", "JJR", "less", "than", "below", "under"],
]
non_triggers["all_greater"] = [
    ["all", "every", "each"],
    ["RBR", "JJR", "more", "than", "above", "after", "exceed", "over"],
]
non_triggers["all_str_eq"] = ["all", "every", "each", "always"]

non_triggers["all_str_not_eq"] = [
    ["all", "every", "each", "always"],
...
non_triggers["all_less_eq"] = [
    non_triggers["filter_less_eq"],
    ["all", "while", "every", "each"],
]
non_triggers["all_greater_eq"] = [
    non_triggers["filter_greater_eq"],
    ["all", "while", "every", "each"],
]

non_triggers["inc_num"] = [
    "and",
    "while",
    "when",
    ",",
...
non_triggers["within_s_s"] = ["within", "one of", "among", "as"]
